...
## Decision 1: Implementing a Centralized Logging and Telemetry Sidecar
...
Specifically, within our current distributed ecosystem, we are observing a critical friction point that demands immediate architectural remediation: the fragmentation of telemetric data and logging, where polyglot services are utilizing inconsistent logging formats, leading to dropped logs, unparseable telemetry, and severe delays in root-cause analysis during high-severity production incidents.
...
We are systematically stripping all log-forwarding, formatting, and external telemetric transmission logic from the application codebases. Applications will now solely be responsible for writing unstructured or semi-structured logs to standard output (stdout) or a localized, shared file volume. Alongside every application container, we are deploying a lightweight log-shipper sidecar (specifically utilizing Fluent Bit). This helper container is exclusively responsible for tailing the local log files, parsing the outputs, enriching the data with cluster-specific metadata (such as Pod IDs, namespaces, and node topology), and asynchronously transmitting the standardized logs to our centralized Elasticsearch and monitoring backends. This decision completely isolates the application's computational threads from the latency and unreliability of the external logging infrastructure.
...
Specific Rationale for the Logging Sidecar: We chose the sidecar pattern for centralized logging over a Node-level DaemonSet primarily to ensure strict tenant isolation and granular configuration management. While a single logging daemon per physical node is slightly more resource-efficient, it forces all logs from all applications into a single processing pipeline. If one extremely chatty application floods the node-level daemon, it can cause buffer overflows and log dropping for every other application on that node. By utilizing a dedicated sidecar per application, we isolate the logging buffers. Furthermore, this allows individual product teams to customize their specific sidecar configurations (for example, masking sensitive PII data using specific regex parsers) without requiring cluster-wide configuration changes that might inadvertently disrupt other services.

### Consequences

Pros - What becomes easier?

- Decoupled Observability and Logging Standardization: Implementing a unified, cluster-wide observability strategy becomes easier and more reliable. Without sidecars, standardizing log formats and ensuring reliable delivery requires integrating complex logging frameworks into every microservice. With the sidecar pattern, the primary application simply writes raw output locally. The sidecar container handles parsing, enrichment, and transmission, offloading all data processing and preventing external network fluctuations from blocking application threads.
...
### Sample Code
Implementing a Centralized Logging Sidecar (Fluent Bit).

```yaml
apiVersion: v1
kind: Pod
metadata:
  name: billing-service-with-logging
  labels:
    app: billing
spec:
  ...
  containers:
    ...
    - name: billing-engine-app
      image: enterprise/billing-engine:v4.2.0
      # The application writes blindly to the local disk, entirely
      # unaware of the complex logging infrastructure.
      command: ["sh", "-c", "while true; do echo \"{\\\"tx_id\\\":\\\"123\\\", \\\"status\\\":\\\"success\\\"}\" >> /var/log/app/transactions.log; sleep 1; done"]
      volumeMounts:
        - name: shared-logs-volume
          mountPath: /var/log/app
